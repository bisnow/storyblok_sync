/**
 * Story reference mapper — ported (nearly verbatim) from the monoblok CLI's
 * `stories/ref-mapper.ts`. Rewrites a dev story into a prod story by remapping
 * every cross-space reference using the in-memory maps:
 *
 *   - story-level `id` / `uuid` / `parent_id` / `alternates`
 *   - content fields, dispatched by the component schema's field type:
 *       asset / multiasset (id + filename + meta_data, S3→CDN normalized),
 *       multilink (story id), bloks (recurse), richtext (story-link uuids +
 *       embedded bloks), options (internal_stories ids).
 *
 * Pure: depends only on the maps, the schemas, and `normalizeAssetUrl`.
 */
import { normalizeAssetUrl } from '@storyblok/management-api-client';
import type { ComponentSchema, ComponentSchemas, RefMaps, Story } from '../types';
...
const traverseAndMapBySchema = (
  ...
  for (const [fieldName, fieldValue] of Object.entries(data)) {
    const fieldSchema = schema[fieldName.replace(/__i18n__.*/, '')] as Record<string, any>;
    ...
    if (fieldRefMapper) {
      dataNew[fieldName] = fieldRefMapper(fieldValue as any, {
        schema: fieldSchema as ComponentSchema,
        schemas,
        maps,
        fieldRefMappers,
      });
...
const multilinkFieldRefMapper: RefMapper = (data, { maps }) => {
  ...
  return {
    ...data,
    id: maps.stories?.get(data.id as any) || data.id,
  };
};
 ...
 * so the Storyblok Image Service (/m/...) works correctly.
 */
...
const optionsFieldRefMapper: RefMapper = (data, { schema, maps }) => {
  if ((schema as Record<string, any>).source !== 'internal_stories' || !Array.isArray(data)) {
    return data;
  }
...
const fieldRefMappers: FieldRefMappers = {
...
/** Story field reference mapper: rewrites a dev story for the prod space. */
export const storyRefMapper = (story: Story, { schemas, maps }: {
  schemas: ComponentSchemas;
  maps: RefMaps;
}): Story => {
  const alternates = story.alternates
    ? story.alternates.map((a: any) => ({
        ...a,
